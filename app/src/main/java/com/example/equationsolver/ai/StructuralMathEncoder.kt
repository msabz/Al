...
object StructuralMathEncoder {
    ...
    fun classify(raw: String): EquationFamily {
        ...
        val maxDegree = Regex("[xy]\\^(\\d+)").findAll(s)
            .mapNotNull { it.groupValues.getOrNull(1)?.toIntOrNull() }
            .maxOrNull() ?: if (s.contains('x') || s.contains('y')) 1 else 0
        return if (maxDegree <= 1) EquationFamily.LINEAR else EquationFamily.POLYNOMIAL
    }
